...
class AttendanceModel {
  ...
  static async isDiscussionAttendanceActive(semester, actual_date) {
    const query = `
  ...
  static async markDiscussionAttendance(semester, studentId, actual_date, status = 'present') {
    const isActive = await this.isDiscussionAttendanceActive(semester, actual_date);
    if (!isActive) {
      throw new Error('Attendance submission is not currently open for this session');
    }
